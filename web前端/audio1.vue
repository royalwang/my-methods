<template>
  <div>
    ...
    <div class="audio-btn">
      <i class="icon icon-prev" @click="switchAudio('top')">上</i>
      <i :class="play ? 'icon icon-stop' : 'icon icon-play'" @click="audioState"></i>
      <i class="icon icon-next" @click="switchAudio('bottom')">下</i>
    </div>
    <div>
...
@function px2rem($px) {
  @return $px/20+rem; //这句是不是感觉很熟悉 这句其实跟上面的那段js是对应的 
}
